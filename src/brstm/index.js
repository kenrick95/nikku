...
export class Brstm {
  ...
  getBuffer(offset, size) {
    ...
    if (this._currentCachedBlock !== offset/samplesPerBlock | 0) {
      //Decode block
      ...
      if(this._partitionedAdpcChunkData == false) {
        console.log('fill partitionedAdpcChunkData')
       ...
       * @var {Array<Uint8Array>} dataChunkData array of non-interlaced raw data; each array represents one channel
       */
      const blockData = this._getPartitionedBlockData(offset/samplesPerBlock | 0);
      
      //Channel info
      if(this._cachedChannelInfo == false) {
        console.log('fill ChannelInfo');
      ...
      if(this._cachedBlock == false) {
        console.log('new cachedBlock array');
        for (let c = 0; c < numberChannels; c++) {
      ...
      for (let c = 0; c < numberChannels; c++) {
        ...
        if (codec === 2) {
          ...
          sampleIndex++
            ...
            this._cachedBlock[c][sampleIndex] = getInt16(blockData[c][sampleIndex]);
          }
        } else {
          throw new Error('Invalid codec');
        }
    ...
    if(this._returnBuffer) {
      ...
      for(let c = 0; c < numberChannels; c++) {
        ...
        for(let p = 0; p < size; p++) {
          ...
          result[c][p] = this._cachedBlock[c][dataIndex+p];
        }
      }
      if(blockEndReached) {
        ...
        for(let c = 0; c < numberChannels; c++) {
          ...
          for(let p = blockEndReachedAt; p < size; p++) {
            result[c][p] = this._cachedBlock[c][dataIndex++];
          }
        }
      }
